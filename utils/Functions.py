...
def merge_image_with_mask(image, mask_image, convert_to_rim=False, save_path=None, show=True, high_resolution=True, color='R'):
    temp = np.array(image)
    ...
    if save_path is not None:
        image_save(temp, save_path, high_resolution=high_resolution)
...
def center_loc(input_mask, axis_list=None):
    ...
    :param axis_list: which axis to return, None for all axis
...
def merge_image_with_mask_3d(image_array, mask_array, save_path=None, high_resolution=True):
    ...
    :return:
    """
    z_to_plot = int(np.median(list(set(np.where(mask_array > 0.5)[2]))))
    merge_image_with_mask(cast_to_0_1(image_array[:, :, z_to_plot]), mask_array[:, :, z_to_plot],
                          save_path=save_path, high_resolution=high_resolution)
